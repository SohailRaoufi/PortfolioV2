...
const GITHUB_LINK = command.social.github.startsWith("http")
  ? command.social.github
  : `https://github.com/${command.social.github}`;
const LINKEDIN_LINK = command.social.linkedin.startsWith("http")
  ? command.social.linkedin
  : `https://www.linkedin.com/in/${command.social.linkedin}`;
const EMAIL = command.social.email.replace(/^mailto:/, "");
...
function commandHandler(input : string) {
  ...
  switch(input) {
    ...
    case 'skills':
      if (bareMode) {
        writeLines(["No skills in bare mode.", "<br>"]);
        break;
      }
      writeLines(SKILLS);
      break;
    case 'awards':
      if (bareMode) {
        writeLines(["No awards in bare mode.", "<br>"]);
        break;
      }
      writeLines(AWARDS);
      break;
    case 'education':
      if (bareMode) {
        writeLines(["No education records in bare mode.", "<br>"]);
        break;
      }
    ...
    case 'contact':
      if (bareMode) {
        writeLines(["No contact info in bare mode.", "<br>"]);
        break;
      }
      writeLines(CONTACT);
      break;
    case 'mission':
      if (bareMode) {
        writeLines(["No mission in bare mode.", "<br>"]);
        break;
      }
      writeLines(MISSION);
      break;
    case 'repo':
      ...
      break;
    case 'linkedin':
      writeLines([`Opening <a href='${LINKEDIN_LINK}' target='_blank'>LinkedIn</a>...`, "<br>"]);
      setTimeout(() => {
        window.open(LINKEDIN_LINK, '_blank');
      }, 500);
      break;
    case 'github':
      writeLines([`Opening <a href='${GITHUB_LINK}' target='_blank'>GitHub</a>...`, "<br>"]);
      setTimeout(() => {
        window.open(GITHUB_LINK, '_blank');
      }, 500);
      break;
    case 'email':
      writeLines([`Opening mail client for <a href='mailto:${EMAIL}'>${EMAIL}</a>...`, "<br>"]);
      setTimeout(() => {
